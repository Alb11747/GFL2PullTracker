...
**Release gates:** This release has no enabled production game-account ownership verifier. Server history submission and recovery fail closed until an audited provider adapter establishes credential-to-account binding. The bounded server relay works without those features. Decoding token metadata or accepting a supplied UID is insufficient proof. Each deployment needs real Google OAuth and authenticated upstream import tests. The earlier hosted build passed Google authorization, upload, automatic sync after import, and reconnect read-back; that evidence does not validate the stable v1 sync. See [live validation limits](docs/GOOGLE_DRIVE.md#live-validation).

Browser-only imports contact the official game API directly. Choosing server features or the explicit relay fallback sends the capture through server memory; captures are never retained. The combined **Contribute to community statistics / Save server backup** choice defaults on when available; an explicit opt-out is remembered and stops future submissions. **Recover a private server backup** defaults off and runs without submitting history. Uploaded histories explicitly submitted after account verification and server-collected histories share one normalized store for recovery and aggregate statistics. Uploaded records are not independently verified for authenticity. See [data and privacy](docs/PRIVACY.md), [import formats](docs/IMPORTING.md), and [preparing a publication copy](docs/PUBLICATION.md).
...
Public mode uses separate routes, session-scoped relay jobs, and a fresh isolated `public-v2.sqlite3` database. Legacy `public.sqlite3` is preserved without migration. It never exposes the desktop owner's API or database. The single-process public API includes rate/resource limits and one account-scoped normalized history store for backups and aggregate statistics, retaining source and fingerprint provenance. **Delete server history** removes both uses while leaving browser and Drive copies untouched. See the release gates above before enabling ownership-dependent features.
