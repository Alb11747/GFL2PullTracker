...
### Reward history rarity toolbar

The history title and reward count share a wrapping horizontal toolbar with the sort order and joined rarity filter bar. Rarity segments retain native multi-select checkbox semantics, charcoal selected backgrounds, white labels, and visible keyboard focus. The All segment selects every available rarity from a partial or empty selection and clears the selection when all are selected. Unknown remains conditional on the existing data and selection rules. On narrow screens the controls wrap below the title; segments retain at least 44px touch targets.
